...
/*  Copyright 2019  闪电  (email : jasperlui@163.com)
...
{
    add_option("SpiderDisplayVersion", "2");
    global $wpdb;
...
function sd_spiderdisplay_general(){
    ...
    <?php
    $spidercountall = $wpdb->get_results("SELECT * , COUNT(*) as 'allcount' FROM  `wp_spiderdisplay`  WHERE `visittime` > '" . date("Y-m-d H:i:s",strtotime("-$deday day")) . "' GROUP BY `spidername` ORDER BY  `allcount` DESC");  
    ?>
    <table border="1">
        <tr>
            <th><?php _e("SearchEngine","SpiderDisplay"); ?></th>
